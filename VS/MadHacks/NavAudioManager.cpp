...
void NavAudioManager::FadeAudioFrames()
{
	...
	// check if there are enough sounds left
	while ((std::chrono::duration_cast <std::chrono::microseconds> (captureTime - soundFrame.timestamp)) > FADE_TIME_US )
	{
		// recycle the oldest sounds
		spAudioPlayer->FreeSounds(spFrames[0].spSounds);
	...
	// now I need to remove the frames that are too old
	spFrames->erase( firstOld, spFrames->rend());
}
